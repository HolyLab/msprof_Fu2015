...
%        drdata.stimlabel(keepdr) == concdata.stimtag(mapc2dr(~isnan(mapc2dr))
%     where keepdr is true for the entries in stimlabel that are common to
%     both.
%
...
% Copyright 2009-2010 by Timothy E. Holy
  ...
  % concdata
  mapc2dr = nan(1,length(flagdr));
  mapc2dr(flagdr) = findainb(drdata.stimlabel(flagdr),concdata.stimtag);
  % Check that mapc2dr is correct
  sl = drdata.stimlabel(flagdr);
  cl = concdata.stimtag(mapc2dr(~isnan(mapc2dr)));
  for i = 1:length(sl)
    if sl{i} ~= cl{i}
      error('Do not line up')
    end
  end
