function h = msplot(scan,plotparams)
  ...
  switch plotparams.type
    ...
    case 'totI'
      ...
      if strcmp(plotparams.xaxis,'gradient')
        x = pB(sflag);
      else
        x = t(sflag);
      end
    ...
    case 'sum-scan'
      ...
      Itot = accumarray(ceil(mzI(:)/plotparams.decimate),I(:));
      h = plot(mzLookup(1:plotparams.decimate:end),Itot);
%       h = fillmm2(zeros(1,length(Itot)),Itot',mzLookup);
      if plotparams.show_xlabel
        xlabel('m/z')
      end
